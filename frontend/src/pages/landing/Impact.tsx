...
export default function Impact() {
  return (
    <Section id="impact" bg="#F8F7FF" className="py-[120px]">
      <div className="max-w-[1200px] mx-auto px-6 lg:px-10">
        <div className="text-center mb-14">
          <SectionLabel className="mb-4" color="#6B4EE6">Impact</SectionLabel>
          <motion.h2
            variants={fadeUp}
            className="text-[clamp(28px,4vw,40px)] leading-tight font-semibold text-[#1A1A2E]"
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-0 lg:divide-x divide-surface-200 mb-16">
          <Counter value={90} suffix=" sec" color="#6B4EE6" label="Average history-taking time, down from 5 minutes." />
          <Counter value={80} suffix="%" color="#2DD4BF" label="History completeness score, up from 45%." />
          <Counter value={12} suffix="" color="#FF8C69" label="Indian languages supported natively." />
        ...
        <motion.div variants={fadeUp} className="bg-white rounded-2xl p-8 shadow-lg shadow-surface-900/5 max-w-[760px] mx-auto">
          <div className="text-center text-lg font-semibold text-[#1A1A2E] mb-8">Before vs After MediKiosk</div>
          ...
          <div className="mt-6 flex items-center justify-center gap-6 text-[13px] text-surface-500">
            ...
            <span className="flex items-center gap-2"><span className="w-3 h-3 rounded-sm bg-gradient-to-r from-primary-600 to-teal-500" /> After</span>
          </div>
        </motion.div>
      </div>

      {/* guardian success floats on the right on large screens */}
      <div className="hidden xl:block absolute right-8 top-1/2 -translate-y-1/2 w-[240px] opacity-90">
  ...
  return (
    <div ref={ref}>
      ...
      <div className="flex items-center gap-3 mt-1.5">
        <span className="w-12 text-[12px] text-primary-600 text-right">{after}%</span>
        <div className="flex-1 h-3.5 rounded-full bg-surface-100 overflow-hidden">
          <motion.div
