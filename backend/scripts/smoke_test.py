...
def main() -> int:
    ...
    try:
        ...
        if not args.skip_db:
            ...
            check("create patient", r.status_code in (200, 201),
                  f"code={r.status_code} body={r.text[:160]}")
            ...
            if pid:
                ...
                check("create session", r.status_code in (200, 201),
                      f"code={r.status_code} body={r.text[:160]}")
                ...
                if sid:
                    ...
                    r = client.post(f"/api/v1/consent/submit", json={
                        "session_id": sid, "consents": [{"consent_type": "data_capture", "granted": True}],
                    })
                    check("consent submit", r.status_code in (200, 201))

            # 2b) F6 erasure request → approve → hard delete (full DB-backed flow)
            if pid:
                r = client.post("/api/v1/advanced/retention/request-erasure", json={
                    "patient_id": pid, "data_types": ["voice_recording"], "requested_by": "smoke-test",
                })
                check("F6 erasure request", r.status_code in (200, 201),
                      f"code={r.status_code} body={r.text[:160]}")
                r = client.delete("/api/v1/advanced/retention/erase-patient", json={
                    "patient_id": pid, "reason": "smoke-test right-to-erasure", "approval": True,
                })
                check("F6 erase-patient", r.status_code == 200 and r.json().get("status") == "erased",
                      f"code={r.status_code} body={r.text[:160]}")
                r = client.get("/api/v1/patients/{pid}".format(pid=pid))
                ...
                check("F6 retention run (dry)", r.status_code == 200 and r.json().get("dry_run") is True,
                      f"code={r.status_code} body={r.text[:160]}")
        ...
        r = client.post("/api/v1/advanced/ml/predict-priority", json={
            "age": 58, "spo2": 88, "pulse": 112, "bp_systolic": 150, "bp_diastolic": 95,
            "red_flag_count": 1, "critical_symptom_count": 1, "has_chest_pain": True,
        })
        check("ML predict-priority", r.status_code == 200 and "priority_class" in r.json(),
              f"code={r.status_code} body={r.text[:160]}")

        r = client.get("/api/v1/advanced/ml/dataset")
        check("ML dataset info", r.status_code == 200 and "real_samples" in r.json())

        r = client.post("/api/v1/advanced/qr/create", json={
            "token_number": "OPD-123", "patient_name": "Smoke", "department": "cardiology", "priority": 3,
        })
        check("F3 QR create", r.status_code == 200 and r.json().get("qr_code_data", "").startswith("MEDIKIOSK|"))

        r = client.post("/api/v1/advanced/emergency/verify", json={
            "alert_type": "test", "symptoms": ["chest_pain"], "vitals": {"spo2": 90},
        })
        ...
        r = client.post("/api/v1/advanced/ml/samples", json={"samples": [
            {"age": 30, "spo2": 97, "pulse": 72, "bp_systolic": 118, "bp_diastolic": 78,
             "temperature": 36.7, "red_flag_count": 0, "critical_symptom_count": 0,
             "has_chest_pain": False, "has_breathlessness": False, "priority_class": "low"},
            {"age": 60, "spo2": 89, "pulse": 115, "bp_systolic": 170, "bp_diastolic": 105,
             "temperature": 38.6, "red_flag_count": 1, "critical_symptom_count": 1,
             "has_chest_pain": True, "has_breathlessness": True, "priority_class": "critical"},
        ]})
        check("ML ingest samples", r.status_code == 200 and r.json().get("accepted") == 2,
              f"code={r.status_code} body={r.text[:160]}")
        ...
        check("ML train gate (expects ≥20 real)", r.status_code in (200, 422),
              f"code={r.status_code} body={r.text[:160]}")
